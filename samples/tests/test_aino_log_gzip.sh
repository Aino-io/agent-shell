...
if [ "$AINO_API_KEY" != "" ]; then
    ...
    export AINO_DISABLE_GZIP="false"
    ...
    if [ "`grep batchId curl.out`" != "" ]; then
        record_success testAinoResponseGzipWith${1}
    else
        record_failure testAinoResponseGzipWith${1} "Wrong response from aino.io, expected to contain batchId" "$CURL_OUT"
        EXIT_CODE=1
    fi
fi